...
class filter
{
	...
	protected static $directory = "app/models/filters/";
	...
	{
		shell::write(" description", "white").shell::write(" - ").shell::write("This command is used to manage filter model classes. \n", "dark_grey");
	}
	...
	{
		shell::write(" make", "light_green").shell::write("        - ").shell::write("Used to generate a filter model class. \n", "light_red");
	...
	public static function make($name = '', $type = 'empty')
	{
		...
		{
			case 'empty':
				$template = app::view()->make('templates.code.filter.filter-empty', ['name' => $name]);
	...
	{
		shell::write(" delete", "light_green").shell::write("      - ").shell::write("Used to delete a filter model class. \n", "light_red");
